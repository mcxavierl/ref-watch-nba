...
const PRE_MERGE_CHECKLIST: Array<{ step: string; command: string; ciStep: string }> = [
  ...
  { step: "Enterprise API route contracts", command: "npm run check:enterprise-api-routes", ciStep: "Enterprise API route contracts" },
  ...
  { step: "Post-build typecheck", command: "npm run typecheck", ciStep: "Post-build typecheck" },
  { step: "Design ship", command: "npm run audit:design-ship", ciStep: "Design ship audit" },
  ...
  { step: "Insight-first", command: "npm run audit:insight-first", ciStep: "Insight-first audit" },
...
function parseWorkflowContracts(): WorkflowContract[] {
  const files = ["ci.yml", "deploy.yml", "morning-slate-poller.yml", "nightly-slate.yml", "refresh-sports-data.yml", "volume-regression.yml"];
