...
export function normalizeEplAbbr(abbr: string): string {
  const upper = abbr.toUpperCase();
  return ESPN_ABBR[upper] ?? upper;
}
...
export function inferEplSeason(espnSeasonYear: number): string {
...
export interface EplGameSummary {
  gameId: string;
  date: string;
  season: string;
  awayAbbr: string;
...
export async function fetchEplScoreboard(
  yyyymmddDate: string,
): Promise<EplScoreboardEvent[]> {
  const res = await fetch(`${SCOREBOARD_BASE}/scoreboard?dates=${yyyymmddDate}`);
  if (!res.ok) {
    throw new Error(`ESPN EPL scoreboard ${yyyymmddDate}: ${res.status}`);
  }
  const body = (await res.json()) as {
    events?: {
      id: string;
  ...
  const events: EplScoreboardEvent[] = [];
  for (const event of body.events ?? []) {
    ...
    for (const team of comp.competitors ?? []) {
      const abbr = normalizeEplAbbr(team.team?.abbreviation ?? "");
...
export async function fetchEplSummary(
  eventId: string,
  espnSeasonYear: number,
): Promise<EplGameSummary | null> {
  ...
  return {
    ...
    season: inferEplSeason(espnSeasonYear),
    awayAbbr,
    homeAbbr,
    awayScore,
    homeScore,
...
export function toEplOfficials(
  officials: { fullName: string }[],
...
export function lookupRosterNumber(
  name: string,
  roster: Map<string, number>,
): number {
  return roster.get(normalizeName(name)) ?? 0;
}
