...
export async function fetchLaligaScoreboard(
  yyyymmddDate: string,
): Promise<LaligaScoreboardEvent[]> {
  const res = await fetch(`${SCOREBOARD_BASE}/scoreboard?dates=${yyyymmddDate}`);
  if (!res.ok) {
    throw new Error(`ESPN La Liga scoreboard ${yyyymmddDate}: ${res.status}`);
  }
  ...
  const events: LaligaScoreboardEvent[] = [];
