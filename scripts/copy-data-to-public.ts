...
copyPair(path.join(root, "data/wnba"), path.join(root, "public/data/wnba"), "ref-photos");
copyPair(path.join(root, "data/cfb"), path.join(root, "public/data/cfb"), "ref-photos");
...
function writeLiveHeaderLeagues(ncaaLive: readonly ("cbb" | "cfb")[]): void {
  const leagues = [...PRO_ONLY_LIVE_LEAGUE_IDS, ...ncaaLive];
...
function writeNcaaLiveLeagues(ncaaLive: readonly ("cbb" | "cfb")[]): void {
  const outPath = path.join(root, "src/lib/ncaa-live-leagues.generated.ts");
  const body = `/** Generated by scripts/copy-data-to-public.ts — do not edit. */
export const NCAA_LIVE_LEAGUE_IDS = ${JSON.stringify([...ncaaLive], null, 2).replace(/\n/g, "\n")} as const;
`;
  fs.writeFileSync(outPath, body);
  console.log(
    `Wrote ${outPath}: ${ncaaLive.length ? ncaaLive.join(", ") : "none (integrity gate failed)"}`,
  );
}
...
const ncaaLiveLeagues = resolveVerifiedNcaaLiveLeagues();
