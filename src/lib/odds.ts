...
import { matchTeamString } from "@/lib/teams";
...
const EMPTY_ODDS: OddsFile = {
  ...
  source: "benchmark",
  note: "No odds file. Set ODDS_API_KEY and run npm run fetch-odds, or we use the 225 league proxy.",
  lines: [],
};
...
/** Match assignment team strings to a sportsbook total when available. */
export function findOddsTotal(
  ...
  return odds.lines.find((line) => {
    ...
    return (
      lineAway?.abbr === away.abbr && lineHome?.abbr === home.abbr
    );
  });
}
