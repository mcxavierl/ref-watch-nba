...
function buildNflFlagsOutlierFinding(stats: RefStatsFile): ScoredFindingBase | null {
  ...
  return { id: "cfb-flags-outlier", category: "whistle-extreme", headline: `${best.name} flag pace outlier`, summary: `${best.name}: ${a.avgFlagsPerGame} flags/game.`, explainer: "CFB historical flags.", stats: [{ label: "Flags", value: String(a.avgFlagsPerGame), detail: `${formatSigned(a.flagsDelta)} vs ${leagueAvg}` }], sampleNote: `${best.games} games`, links: [{ label: best.name, href: `/cfb/refs/${best.slug}` }], score: rankScore(Math.abs(a.flagsDelta)/leagueAvg, best.games, MIN_REF_GAMES), sampleGames: best.games };
}
