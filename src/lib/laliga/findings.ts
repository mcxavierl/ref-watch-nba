import { formatPct, formatSigned, getRefStats, getTeamSplits } from "@/lib/laliga/data";
import { buildScopedRefStats } from "@/lib/scoped-ref-stats";
import { formatPctFromWlp } from "@/lib/ref-betting";
import { getTeam, teamFullName, LALIGA_TEAMS } from "@/lib/laliga/teams";
...
function teamCrewAnomalyFinding(stats: RefStatsFile): ScoredFindingBase | null {
  ...
  return {
    ...
    links: [{ label: teamName, href: `/laliga/teams/${best.team}` }],
...
function ouAtsEdgeFinding(stats: RefStatsFile): ScoredFindingBase | null {
  ...
  return {
    ...
    links: [{ label: best.ref.name, href: `/laliga/refs/${best.ref.slug}` }],
...
function scoringExtremesFinding(stats: RefStatsFile): ScoredFindingBase | null {
  ...
  return {
    ...
    links: [
      { label: hottest.ref.name, href: `/laliga/refs/${hottest.ref.slug}` },
      { label: hotName, href: `/laliga/teams/${hottest.team}` },
    ],
...
const LALIGA_FINDING_CTX: LeagueFindingContext = {
  league: "LALIGA",
  paths: {
    idPrefix: "laliga-",
    refsBrowsePath: "/laliga/refs",
    refPath: (slug) => `/laliga/refs/${slug}`,
    teamPath: (abbr) => `/laliga/teams/${abbr}`,
    matrixPath: "/laliga/matrix",
    crewsPath: "/laliga/crews",
    trendsPath: "/laliga/trends",
  },
  labels: {
    scoreUnit: "goals",
    whistleUnit: "fouls",
...
function scoringOutlierFinding(stats: RefStatsFile): ScoredFindingBase | null {
  ...
  return {
    ...
    links: [{ label: ref.name, href: `/laliga/refs/${ref.slug}` }],
...
function buildEplFoulsOutlierFinding(stats: RefStatsFile): ScoredFindingBase | null {
  ...
  return {
    ...
    sampleNote: formatFindingSampleMeta(best.games, stats.meta.seasons),
    links: [{ label: best.name, href: `/laliga/refs/${best.slug}` }],
...
function collectCandidates(
  ...
  return [
    ...
    ...(options?.hub ? [] : [buildLeagueSkewFinding(stats, LALIGA_FINDING_CTX)]),
    ouAtsEdgeFinding(stats),
    atsOutlierFinding(stats),
    buildYoYTrendFinding(stats, LALIGA_FINDING_CTX),
    buildWhistleOutlierFinding(stats, LALIGA_FINDING_CTX),
    buildOverRateOutlierFinding(stats, LALIGA_FINDING_CTX, "low"),
    buildOverRateOutlierFinding(stats, LALIGA_FINDING_CTX, "high"),
    scoringOutlierFinding(stats),
    buildMarqueeEfficiencyFinding(stats, LALIGA_FINDING_CTX),
    ...buildFrictionGrudgeFindings(stats, LALIGA_FINDING_CTX, "laliga"),
    ...(includeHeavy
      ? [
          teamCrewAnomalyFinding(stats),
          scoringExtremesFinding(stats),
          buildMatrixExtremeFinding(stats, LALIGA_FINDING_CTX, "high"),
          buildMatrixExtremeFinding(stats, LALIGA_FINDING_CTX, "low"),
          buildCloseGameLeagueFinding(stats, LALIGA_FINDING_CTX),
          buildTeamHomeRoadFinding(stats, LALIGA_FINDING_CTX),
        ]
      : []),
...
function resolveStats(scopedSeasons?: string[]) {
  ...
  return prepareStatsForAtsAnalytics(
    "laliga",
    buildScopedRefStats("laliga", full, scopedSeasons),
    scopedSeasons,
  );
}
