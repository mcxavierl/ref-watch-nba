...
export const METHODOLOGY_SECTIONS: MethodologySection[] = [
  ...
  },
  {
    ...
    bullets: [
      `Game-State Index: compares an official's penalty frequency in matched score-and-clock buckets to the league average in those same states. Reported as an Index Score: positive is below-average penalty frequency, negative is above-average frequency, zero is typical. Frequency bands: Below-Average, Typical, Above-Average. NBA requires ${GSNI_MIN_HIGH_LEVERAGE_MINUTES}+ high-leverage minutes; NFL requires ${GSNI_MIN_HIGH_LEVERAGE_MINUTES_NFL}+. Research highlights need ${GSNI_RESEARCH_MIN_SAMPLE_GAMES}+ games and an extreme band after Bayesian shrinkage.`,
      "Index Score bands: we label outliers when the absolute score exceeds the configured extreme threshold. Game-State Index uses shrinkage toward the league average so thin samples regress before ranking.",
