...
import { detectTeamsInGame, EPL_TEAM_ABBRS } from "@/lib/epl/teams";
import {
  computeOuLean,
  formatPct,
  formatSigned,
...
function resolveTeamSplits(
  ...
  return resolveTeamSplitsForLeague("epl", embedded, diskTeamSplitsFallback(loadTeamSplitsFromDisk));
}

function loadRefStatsRaw(): RefStatsFile | null {
  return loadRefStatsRawCachedFirst("epl", () => {
    const bundled = getBundledLeagueRefStatsCore("epl");
...
export function computeCrewMetrics(
  ...
  if (qualified.length === 0) {
    return {
      ...
      provenance: eplCrewMetricsProvenance(stats, 0, 0),
    };
  }
  ...
  return {
    ...
    provenance: eplCrewMetricsProvenance(
      stats,
      qualified.length,
      avgSampleGames,
    ),
...
export function getAllTeamAbbrs(): string[] {
  ...
  return [...EPL_TEAM_ABBRS];
}

export { detectTeamsInGame, getTeam, matchTeamString } from "@/lib/epl/teams";
export type { EplTeam } from "@/lib/epl/teams";
