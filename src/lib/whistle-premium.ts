...
function buildAlert(
  ...
  if (
    ...
    return {
      alert: "high_pace",
      reason: `Crew scoring premium ${formatSigned(premium.scoringPremium)} vs league; historical avg ${premium.avgTotalPoints} sits ${formatSigned(premium.gapVsBenchmark)} above ${bench}. Historical association only — not a bet recommendation.`,
    };
  }
  ...
  if (
    ...
    return {
      alert: "low_pace",
      reason: `Crew scoring premium ${formatSigned(premium.scoringPremium)} vs league; historical avg ${premium.avgTotalPoints} sits ${formatSigned(premium.gapVsBenchmark)} below ${bench}. Historical association only — not a bet recommendation.`,
    };
  }
...
export function premiumHeadline(premium: CrewWhistlePremium): string {
  ...
  return `${label}: ${formatPremiumLabel(premium.scoringPremium)} scoring · ${formatSigned(premium.foulPremium)} fouls`;
}
