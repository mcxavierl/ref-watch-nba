...
import { detectTeamsInGame, NFL_TEAM_ABBRS } from "@/lib/nfl/teams";
import {
  computeOuLean,
  formatPct,
  formatSigned,
...
function resolveTeamSplits(
  ...
  const cached = cachedTeamSplitsForLeague("nfl");
  if (Object.keys(cached).length > 0) {
    return resolveTeamSplitsForLeague("nfl", embedded, cached);
  }
  return resolveTeamSplitsForLeague("nfl", embedded, diskTeamSplitsFallback(loadTeamSplitsFromDisk));
}

function loadRefStatsRaw(): RefStatsFile | null {
  return loadRefStatsRawCachedFirst("nfl", () => {
    if (!allowNodeDataFs()) return getBundledLeagueRefStatsCore("nfl");
    return (
      tryReadJson<RefStatsFile>("ref-stats-core.json") ??
      tryReadJson<RefStatsFile>("ref-stats.json") ??
      getBundledLeagueRefStatsCore("nfl")
    );
  });
}
...
const EMPTY_REF_STATS: RefStatsFile = (() => {
  const bl = resolveLeagueBaseline("NFL");
  return {
    meta: {
      lastUpdated: new Date().toISOString(),
      seasons: [],
...
function applyBaselines(stats: RefStatsFile): RefStatsFile {
  ...
  const baseline = resolveLeagueBaseline("NFL", season);
  return {
    ...stats,
    meta: {
      ...stats.meta,
...
export function getRefStats(): RefStatsFile {
  try {
    ...
    if (resolvedRefStats?.refs?.length) {
      resolvedRefStats = gateUnverifiedNflStats(
...
export function computeCrewMetrics(
  ...
  if (qualified.length === 0) {
    return {
      ...
      provenance: nflCrewMetricsProvenance(stats, 0, 0),
    };
  }
  ...
  return {
    ...
    provenance: nflCrewMetricsProvenance(
      stats,
      qualified.length,
      avgSampleGames,
    ),
...
export function getAllTeamAbbrs(): string[] {
  ...
  return [...NFL_TEAM_ABBRS];
}

export { detectTeamsInGame, getTeam, matchTeamString } from "@/lib/nfl/teams";
export type { NflTeam } from "@/lib/nfl/teams";
