...
import { getRefStats } from "@/lib/cfb/data";
import {
  buildYoYNarrative,
...
export const metadata: Metadata = {
  title: "CFB league trends, 5 seasons",
  description:
    "Five-season CFB scoring, minor penalty, and overtime trends from Ref Watch baselines. Year-over-year context in plain language.",
  alternates: { canonical: absoluteUrl("/cfb/trends") },
};

export default function CfbTrendsPage() {
  const baselines = getBaselinesFile();
  const stats = getRefStats();
  const rows = seasonRowsFromBaselines(baselines.CFB.seasons);
  const narrative = buildYoYNarrative(rows, "CFB");
  const usingFallback = baselineUsingFallback("CFB");
  ...
  return (
    <div className="page-shell">
      ...
      <section className="page-hero">
        <h1 className="page-title">CFB league trends</h1>
      ...
      <section className="section-block">
        ...
        <p className="mt-6 text-sm text-zinc-600">
          Compare officials:{" "}
          <Link href="/cfb/rankings" className="font-medium text-zinc-800 hover:underline">
            CFB referee rankings →
          </Link>
        </p>
      </section>
    </div>
