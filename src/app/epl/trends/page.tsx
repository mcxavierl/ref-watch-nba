...
import { formatPct } from "@/lib/epl/data";
...
export const metadata: Metadata = {
  title: "EPL league trends, 5 seasons",
  description:
    "Five-season EPL scoring, minor penalty, and overtime trends from Ref Watch baselines. Year-over-year context in plain language.",
  alternates: { canonical: absoluteUrl("/epl/trends") },
};

export default function EplTrendsPage() {
  const baselines = getBaselinesFile();
  const stats = getRefStats();
  const rows = seasonRowsFromBaselines(baselines.EPL.seasons);
  const narrative = buildYoYNarrative(rows, "EPL");
  const usingFallback = baselineUsingFallback("EPL");
  ...
  return (
    <div className="page-shell">
      ...
      <section className="page-hero">
        <h1 className="page-title">EPL league trends</h1>
      ...
      <section className="section-block">
        ...
        <p className="mt-6 text-sm text-zinc-600">
          Compare officials:{" "}
          <Link href="/epl/rankings" className="font-medium text-zinc-800 hover:underline">
            EPL referee rankings →
          </Link>
        </p>
      </section>
    </div>
