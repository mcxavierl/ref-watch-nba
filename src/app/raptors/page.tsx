...
import type { OuLean, RaptorsCrewSplit } from "@/lib/types";
...
export default function RaptorsPage() {
  ...
  const splits = stats.raptorsSplits ?? [];
  const totalGames = splits.reduce((s, sp) => s + sp.games, 0);

  return (
    <div className="page-shell">
      <section className="mb-10">
        <p className="text-sm font-semibold text-raptors">Toronto Raptors</p>
        <h1 className="mt-2 text-2xl font-semibold tracking-tight text-zinc-900 sm:text-[1.75rem]">
          Ref crew splits
        </h1>
        <p className="page-lead">
          Crew-specific Raptors history — not league-wide ref profiles. Each
          block covers every TOR game this exact three-ref crew worked across{" "}
      ...
      {splits.length === 0 ? (
        <p className="text-sm text-zinc-500">No Raptors crew splits in dataset yet.</p>
      ) : (
      ...
      <details className="methodology-details panel-inset mt-10 px-5 py-4">
        <summary>How to read Raptors splits</summary>
        <ul className="space-y-2.5 text-sm leading-relaxed text-zinc-600">
          ...
          <li>
            ...
            on Toronto vs opponent per game. Differential ≥+1.5 favors Raptors,
            ≤−1.5 favors opponent.
          </li>
          <li>
    ...
    <article className="data-card">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border bg-surface-raised/60 px-4 py-3">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <WhistleBiasBadge bias={bias} diff={split.foulDifferential} teamAbbr="TOR" />
        </div>
      </div>
      ...
      <StatSection title="Whistle & fouls">
        <StatStrip>
          ...
          />
          <StatCell label="TOR" value={String(split.avgTeamFouls)} />
        ...
        {split.crewNames.map((name) => {
          ...
          return (
            <Link
              key={ref.slug}
              href={`/refs/${ref.slug}`}
              className="text-[11px] text-zinc-600 transition hover:text-raptors"
            >
              {name} →
            </Link>
          );
