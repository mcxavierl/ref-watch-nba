...
export type MatrixRowProps = {
  ...
  foulRatioLabel?: string;
  foulRatioBar?: ReactNode;
};

function formatFoulCount(value: number | null | undefined): string {
...
export function MatrixRow({
  ...
  const showFoulColumns =
    subjectiveFouls != null || adminFouls != null || foulRatio != null;

  return (
    <li
      className={`team-ref-matrix-row analytics-matrix-row py-2 text-sm${expanded ? " analytics-matrix-row--expanded" : ""}${showFoulColumns ? " matrix-table-row--fouls" : ""}`}
    >
      <div className="team-ref-matrix-row-main">
        ...
        {showFoulColumns ? (
          <>
            <span className="team-ref-matrix-stat team-ref-matrix-stat--subjective tabular-nums text-right master-table-metric-secondary">
              {formatFoulCount(subjectiveFouls)}
            </span>
            <span className="team-ref-matrix-stat team-ref-matrix-stat--admin tabular-nums text-right master-table-metric-secondary">
              {formatFoulCount(adminFouls)}
            </span>
            <span className="team-ref-matrix-stat team-ref-matrix-stat--ratio tabular-nums text-right master-table-metric-secondary">
              <span className="matrix-table-ratio-cell">
                <span className="truncate">{foulRatioLabel ?? "n/a"}</span>
                {foulRatioBar}
              </span>
            </span>
          </>
      ...
      {expanded ? (
        <div className="team-ref-matrix-row-details md:hidden">
          ...
          {showFoulColumns ? (
            <>
              <div className="team-ref-matrix-row-details-stat">
                <span className="team-ref-matrix-row-details-label">Subj. fouls</span>
                <span className="team-ref-matrix-row-details-value tabular-nums text-right">
                  {formatFoulCount(subjectiveFouls)}
                </span>
              </div>
              <div className="team-ref-matrix-row-details-stat">
                <span className="team-ref-matrix-row-details-label">Admin fouls</span>
                <span className="team-ref-matrix-row-details-value tabular-nums text-right">
                  {formatFoulCount(adminFouls)}
                </span>
              </div>
              <div className="team-ref-matrix-row-details-stat">
                <span className="team-ref-matrix-row-details-label">Ratio</span>
                <span className="team-ref-matrix-row-details-value tabular-nums text-right">
                  {foulRatioLabel ?? "n/a"}
                </span>
              </div>
