...
export function FoulViewToggle({
  ...
  return (
    <div
      className={`foul-view-toggle shrink-0 ${className}`.trim()}
      role="group"
        ...
        {FOUL_VIEW_OPTIONS.map((option) => {
          ...
          return (
            <button
              key={option.id}
              type="button"
              className={`foul-view-toggle-btn whitespace-nowrap px-3 text-sm tabular-nums${isActive ? " foul-view-toggle-btn-active" : ""}`}
            ...
            >
              ...
              <span className="foul-view-toggle-label truncate">{option.label}</span>
            </button>
          );
        })}
      </div>
