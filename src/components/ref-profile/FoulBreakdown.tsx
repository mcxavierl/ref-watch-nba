  ...
  return (
    ...
    >
      <div className="foul-breakdown-head">
        <div className="foul-breakdown-head-copy min-w-0">
              ...
              return (
                <li key={foul.id} className="foul-breakdown-row">
                  <div className="foul-breakdown-row-main">
                    <span className="foul-breakdown-label truncate">{foul.label}</span>
                    <span className={`${categoryBadgeClass(category)} whitespace-nowrap px-3`}>
                      {categoryLabel(category)}
                    </span>
                  </div>
                  <div className="foul-breakdown-row-meta tabular-nums">
                    <span className="truncate">{foul.matchup}</span>
                    <span>{foul.date}</span>
                    {foul.team ? <span>{foul.team}</span> : null}
                    {foul.yards !== undefined ? (
                      <span className="text-right">{foul.yards} yds</span>
                    ) : null}
                  </div>
                </li>
              );
