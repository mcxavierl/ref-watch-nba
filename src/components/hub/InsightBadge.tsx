import type { LucideIcon } from "lucide-react";
import { Sparkles } from "lucide-react";
import type { ReactNode } from "react";

/**
 * Unified insight/notable pill: Sparkles by default, truncates inside flex rows.
 */
export function InsightBadge({
  label,
  icon: Icon = Sparkles,
  className = "",
}: {
  label: ReactNode;
  icon?: LucideIcon;
  className?: string;
}) {
  return (
    <span
      className={`insight-badge pill-constrain inline-flex min-w-0 items-center gap-1.5 rounded-full border px-4 py-1.5 text-xs font-medium leading-none ${className}`.trim()}
    >
      <Icon className="h-3 w-3 shrink-0 opacity-80" strokeWidth={2.1} aria-hidden />
      <span className="pill-constrain-text">{label}</span>
    </span>
  );
}
